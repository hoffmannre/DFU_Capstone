...
import os
from imageML_subroutines import *

# import image array using subroutine
imageAr = get_images_in_directory("./capstone/imageML/DFU_Images/TestSet")

# import bouning box data and get it in a usable format
bbDF = pd.read_csv("boundingboxes.csv")
bbDF.set_index('name', inplace=True, drop=True)
bbAr = buildBindingBoxAr(imageAr, bbDF)

# label array for images, tells the algorithm that all defined bounding boxes are for DFUs
lbls = []
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # Load image
        img = imageAr[idx]  # Load your image here
        # Load corresponding bounding boxes and labels

        boxes = bbAr[idx]  # Load or define bounding boxes

        labels = lbls[idx]  # Load or define labels
...
# Load image
img = Image.open("C:\Users\hoff6\Documents\VScode\capstone\imageML\DFU_Images\Original Images\\1.jpg")
